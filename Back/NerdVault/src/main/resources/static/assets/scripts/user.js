document.addEventListener("DOMContentLoaded", () => {
  ...
  async function fetchOrders() {
    ...
    fetch(apiUrl)
      ...
      .then((data) => {
        ...
        const speditiOrdini = ordini.filter(
          (ordine) => ordine.statoOrdine === SPEDITO
        );
        const lavorazioneOrdini = ordini.filter(
          (ordine) => ordine.statoOrdine === IN_LAVORAZIONE
        );
        const consegnatiOrdini = ordini.filter(
          (ordine) => ordine.statoOrdine === CONSEGNATO
        );
        const annullatiOrdini = ordini.filter(
          (ordine) => ordine.statoOrdine === CANCELLATO
        );
  ...
  function filterAndDisplayOrders(ordini) {
    ...
    const lavorazioneChecked =
      document.getElementById("CheckLavorazione").checked;
    const consegnatiChecked =
      document.getElementById("CheckConsegnati").checked;
    ...
    console.log(
      "filtri selezionati:" + speditiChecked,
      lavorazioneChecked,
      consegnatiChecked,
      annullatiChecked
    );
